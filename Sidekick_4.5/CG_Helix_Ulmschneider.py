...
if options.batch:
	batch_entry = BatchTools.helix_batch_enter(options)
	import atexit
...
try:
	GromacsInterface.RobustMolecularDynamics(topology_file="ioned_topol",input_structure="em",output_name="intial",seed=seed,steps=1250000,pcoupling=pcoupling,wallclock=options.wallclock,epsilon=System.epsilon,temperature=options.temperature,apolar=System.apolar,pme=System.pme)
except:
	if options.batch:
		sys.exit()

Initial_Analysis = AnalyseSimulation.AnalyseHelix(System)

GromacsInterface.RobustMolecularDynamics(topology_file="ioned_topol",input_structure="initial",output_name="t_0",seed=seed,steps=MDsteps,pcoupling=pcoupling,wallclock=options.wallclock,epsilon=System.epsilon,temperature=500,apolar=System.apolar,pme=System.pme)
